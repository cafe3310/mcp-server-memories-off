...
import {getTocList, matchToc} from "./toc.ts";
import {linesMatchContent, linesReplace, linesVerifyBeginEnd} from "./lines.ts";
import {checks} from "../../utils.ts";

export function readSectionContent(libraryName: LibraryName, fileType: FileType, name: ThingName, tocGlob: TocGlob): string[] {
  const lines = readFileLines(libraryName, fileType, name);
  const tocList = getTocList(libraryName, fileType, name);
  const matchedToc = matchToc(libraryName, fileType, name, tocGlob);
